...
class Database:
    ...
    async def _load_data(self, user_id: int) -> Dict:
        ...
        Caller is expected to hold ``self._lock`` before invoking to avoid nested locking.
        """
    ...
    async def init(self, user_id: int):
        ...
        try:
            async with self._lock:
                ...
                if not self._data.get(user_id_str):
        ...
        try:
            async with self._lock:
                ...
                if file_path.exists():
                    ...
                    file_path.unlink()
                    logger.info(
                        f"Deleted settings for user {user_id} and backed up to {backup_file}"
                    )
    ...
    async def backup_database(self):
        ...
        try:
            async with self._lock:
                ...
                # Ensure in-memory state is flushed before copying
